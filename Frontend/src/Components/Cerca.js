...
class Cerca extends Component {
  ...
  submitHandler = (e) => {
    ...
    const url = 'http://localhost:3000/users/search' + query;
    axios.get(url).then(response=>{
      this.setState({users: response.data.data});
    ...
    return(
     ...
     <font size="3" face='Georgia' color="white"> Vetrina</font>
      ...
      {vetrina.map(user=>(
       ...
       <img src={`http://localhost:3000/users/${user.generalUser._id}/file/${user.generalUser.photo}`} style={{ paddingBottom:0, height:100, width:100}} alt=''  /> 
