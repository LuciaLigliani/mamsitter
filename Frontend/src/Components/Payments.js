...
class Payments extends React.Component {
  ...
  componentDidMount() {
    ...
    window.paypal.Buttons({
      ...
      onApprove: function(data, actions) {
        axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
        return axios.post('http://localhost:3000/payments/highlight', data.subscriptionID).then((response => {
  ...
  window.paypal.Buttons({
    ...
    onApprove: function(data, actions) {
      axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
      return axios.post('http://localhost:3000/payments/canApply', data.subscriptionID).then((response => {
        setTimeout(()=> {
          this.setState({open:false})
             }, 2000);
        setTimeout(()=> {
...
window.paypal.Buttons({
  ...
  onApprove: function(data, actions) {
    axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
    return axios.post('http://localhost:3000/payments/createAnn', data.subscriptionID).then((response => {
      setTimeout(()=> {
        this.setState({open:false})
           }, 2000);
      setTimeout(()=> {
...
window.paypal.Buttons({
  ...
  onApprove: function(data, actions) {
    axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
    return axios.post('http://localhost:3000/payments/base', data.subscriptionID).then((response => {
      this.setState({open:true, message:'Abbonamento effettuato!'})
      setTimeout(()=> {
        this.setState({open:false})
           }, 2000);
...
window.paypal.Buttons({
  ...
  onApprove: function(data, actions) {
    axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
    return axios.post('http://localhost:3000/payments/premium', data.subscriptionID).then((response => {
      this.setState({open:true, message:'Abbonamento effettuato!'})
      setTimeout(()=> {
        this.setState({open:false})
           }, 2000);
