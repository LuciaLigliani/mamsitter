...
componentDidMount(){
  ...
  axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
  axios.get('http://localhost:3000/announcements/' + id).then(response => {
    let specificData;
...
deleteAnn = () => {
  axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
  axios.delete('http://localhost:3000/announcements/' + this.state.id).then(profile => {
...
updateAnn = () => {
  ...
  axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
  axios.patch('http://localhost:3000/announcements/' + this.state.id, this.state).then(response=>{
...
deleteApply = () => {
  const id= this.props.location.pathname.split('/')[2];
  axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
  axios.delete('http://localhost:3000/announcements/'+id+'/applications/').then(response => {
...
apply = () => {
  const id= this.props.location.pathname.split('/')[2];
  axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
  axios.post('http://localhost:3000/announcements/' +id+ '/applications').then(response => {
...
/*deleteApp = () => {
  axios.delete('http://localhost:3000/announcements/' +this.state.id+ '/applications/'+ appId).then(app=> {
    console.log(app);
    this.setState({open:true, message:'Candidatura eliminata'})
      setTimeout(()=> {
        window.location.assign('/viewallapplication');
