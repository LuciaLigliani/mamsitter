...
class ErrorAnn extends Component{
  constructor(props){
    super(props);
    this.state={
      anchorEl:''
  ...
  componentDidMount() {
    ...
    window.paypal.Buttons({
      ...
      onApprove: function(data, actions) {
        axios.defaults.headers.common['Authorization'] = 'Bearer ' + util.getCookie('user_jwt');
        return axios.post('http://localhost:3000/payments/highlight', ).then((data => {
...
<Link to="/"><img src={logomodi} className="navbarLogo" alt="logo"/></Link>
  ...
  <Card style={{top:'120px', width:'50rem', left:'130px', bottom:'-3px' }}>
    ...
    <Card.Title bg='light'>CREA ANNUNCIO</Card.Title>
    <Card.Text>
       Abbonati per avere la possibilità di creare tutti gli annunci che vuoi!<br/>
       Avrai la possibilità di metterti in contatto con i lavoratori che rispecchiano le tue esigenze. <br/>
       Loro potranno candidarsi ad un tuo annuncio e potrete mettervi in contatto!
       </Card.Text>
